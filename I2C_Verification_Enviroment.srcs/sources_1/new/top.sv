...
typedef enum logic [3:0] {idle=0,start=1,write_addr=2,ack_1=3,write_data=4,
read_data=5,stop=6,ack_2=7,master_ack=8} state_type;
...
if(bitcount<=7) begin
case(pulse)
0: begin scl_t<=1'b0; sda_t<=0;end
1: begin scl_t<=1'b0; sda_t<=0;end
...
sda_en<=1'b1;
case(pulse)
0: begin scl_t<=1'b0; sda_t<=1;end
1: begin scl_t<=1'b0; sda_t<=1;end
2: begin scl_t<=1'b1; sda_t<=1;  end
3: begin scl_t<=1'b1; sda_t<=1; end
endcase
if(count1==clk_count1*4-1) begin
sda_t<=1'b0;
state<=stop;
...
sda_en<=1'b1;
case(pulse)
0:begin scl_t<=1'b1; sda_t<=0; end
1:begin scl_t<=1'b1; sda_t<=0; end
2:begin scl_t<=1'b1; sda_t<=1; end
3:begin scl_t<=1'b1; sda_t<=1; end
endcase
...
module i2c_top(input clk,rst,newd,op,
input [6:0] addr,
input [7:0] din,
output [7:0] dout,
output busy,ack_err,
...
wire done_m,done_s;
i2c_master master(clk, rst, newd, addr, op, sda, scl, din, dout, busy,ack_errm , done_m);
i2c_Slave slave (scl, clk, rst, sda, ack_errs,done_s );
